...
export function KnowledgeBasePage() {
  ...
  return (
    <div className="space-y-5">
      {/* Header Banner */}
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-panel">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-sky-50 dark:bg-sky-950 text-sky-600 dark:text-sky-400 border border-sky-100 dark:border-sky-900">
              <BookOpen className="h-5 w-5" />
            </div>
            <div>
              <h1 className="text-xl font-bold text-slate-900 dark:text-white">Knowledge Base &amp; SOP ClO₂</h1>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.3fr)]">
        ...
        >
          {documents.loading && !documents.data ? (
            ...
            <ul className="divide-y divide-slate-100 dark:divide-slate-800 max-h-[580px] overflow-y-auto">
                ...
                <li key={doc.id}>
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="rounded bg-slate-100 dark:bg-slate-800 px-2 py-0.5 text-[10px] font-bold text-slate-600 dark:text-slate-300 shrink-0">
                        {doc.doc_type}
                      </span>
                    </div>
                    {doc.summary && (
        ...
        <Panel eyebrow="Isi Dokumen" title={selected?.title ?? 'Pilih dokumen dari daftar'}>
          {loadingDoc ? (
            ...
            <div className="space-y-4">
              <dl className="grid grid-cols-2 sm:grid-cols-4 gap-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 p-3.5 border border-slate-100 dark:border-slate-800 text-xs">
                <div>
                ...
                <div>
                  ...
                  <dd className="font-semibold text-slate-800 dark:text-slate-200 mt-0.5">{selected.doc_type}</dd>
                </div>
                <div>
                  <dt className="text-[10px] font-bold uppercase text-slate-400">Versi</dt>
                  <dd className="font-mono text-slate-800 dark:text-slate-200 mt-0.5">v{selected.version ?? '1.0'}</dd>
                </div>
                <div>
                  <dt className="text-[10px] font-bold uppercase text-slate-400">Diperbarui</dt>
                  <dd className="text-slate-800 dark:text-slate-200 mt-0.5">{formatDateTime(selected.updated_at)}</dd>
                </div>
              </dl>

              <div className="rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 shadow-xs">
                <p className="whitespace-pre-line text-xs sm:text-sm leading-relaxed text-slate-800 dark:text-slate-200 font-sans">
              ...
              {selected.tags?.length ? (
                <div className="flex flex-wrap items-center gap-1.5 pt-2">
                  ...
                  {selected.tags.map((tag) => (
                    <span
                      key={tag}
                      className="rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800 px-2 py-0.5 font-mono text-[10px] font-semibold text-slate-600 dark:text-slate-400"
                    >
                      #{tag}
                    </span>
                  ))}
