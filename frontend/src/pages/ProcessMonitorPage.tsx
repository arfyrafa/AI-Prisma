...
export function ProcessMonitorPage() {
  ...
  const series: TrendSeries[] = selected.map((name, index) => ({
    parameter: name,
    label: parameters.data?.find((p) => p.parameter_name === name)?.display_name ?? name,
    color: SERIES_COLORS[index % SERIES_COLORS.length],
  }))

  const reference =
    selected.length === 1
      ? (parameters.data?.find((p) => p.parameter_name === selected[0]) ?? null)
      : null
  ...
  return (
    <div className="space-y-6">
      ...
      <Panel eyebrow="Batas operasi" title="Status seluruh parameter">
        <ParameterTable parameters={snapshot?.parameters ?? []} />
      </Panel>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 backdrop-blur-xs p-4 animate-in fade-in">
          <div className="w-full max-w-xl rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
              <div className="flex items-center gap-2.5">
                <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-50 dark:bg-emerald-950 text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-900">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-slate-50 dark:bg-slate-800/60 p-3 border border-slate-200 dark:border-slate-700 text-xs">
              ...
              <div className="flex items-center gap-2">
                <a
                  ...
                  className="rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900 px-2.5 py-1 text-[11px] font-semibold text-sky-600 dark:text-sky-400 hover:underline"
                >
            ...
            <div className="rounded-xl border-2 border-dashed border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/40 p-6 text-center">
              ...
              >
                <UploadCloud className="h-9 w-9 text-emerald-600 dark:text-emerald-400 animate-bounce" />
            ...
            {parsedRows.length > 0 && (
              <div className="space-y-2">
                ...
                <div className="max-h-36 overflow-y-auto rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800 p-2.5 font-mono text-[10px]">
            ...
            {/* Alert Status */}
            {uploadError && (
              <p className="text-xs font-semibold text-rose-600 bg-rose-50 dark:bg-rose-950/40 p-3 rounded-xl border border-rose-200 dark:border-rose-900">
                {uploadError}
              </p>
            )}
            {uploadSuccess && (
              <p className="text-xs font-semibold text-emerald-600 bg-emerald-50 dark:bg-emerald-950/40 p-3 rounded-xl border border-emerald-200 dark:border-emerald-900 inline-flex items-center gap-1.5 w-full">
