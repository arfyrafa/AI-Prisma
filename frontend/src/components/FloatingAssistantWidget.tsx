...
export function FloatingAssistantWidget() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      ...
      {isOpen && (
        <div
          className={`mb-4 flex flex-col overflow-hidden rounded-3xl border border-slate-200/80 dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl shadow-2xl transition-all duration-300 ${
            isExpanded
        ...
        >
          {/* Header */}
          <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 bg-gradient-to-r from-slate-900 via-sky-950 to-slate-900 px-4 py-3.5 text-white">
          ...
          <div className="flex-1 space-y-3.5 overflow-y-auto p-4 text-xs">
            {messages.map((msg, idx) => (
              ...
              >
                {msg.role === 'assistant' && (
                  <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-sky-100 dark:bg-sky-950/80 border border-sky-200 dark:border-sky-800 text-sky-600 dark:text-sky-400 shadow-xs">
            ...
            {sending && (
              <div className="flex items-center gap-2.5 text-slate-400">
                <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-sky-100 dark:bg-sky-950/80 border border-sky-200 dark:border-sky-800 text-sky-600 dark:text-sky-400">
                  <Bot className="h-4 w-4 animate-spin" />
                </div>
                <div className="rounded-2xl rounded-bl-xs border border-slate-200/80 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/80 px-4 py-2.5 shadow-xs">
                  <div className="flex items-center gap-1.5">
          ...
          <div className="border-t border-slate-100 dark:border-slate-800/60 bg-slate-50/60 dark:bg-slate-900/60 px-3 py-2">
            <p className="text-[10px] font-semibold text-slate-400 dark:text-slate-500 mb-1.5 flex items-center gap-1">
            ...
            <div className="flex gap-1.5 overflow-x-auto pb-1 scrollbar-none">
              {QUICK_SUGGESTIONS.map((sug, i) => (
                <button
                  ...
                  disabled={sending}
                  className="whitespace-nowrap rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-2.5 py-1 text-[10px] font-medium text-slate-600 dark:text-slate-300 hover:border-sky-400 hover:text-sky-600 dark:hover:text-sky-400 transition-colors shadow-2xs"
                >
                  {sug}
                </button>
              ))}
          ...
          >
            <input
              ...
              disabled={sending}
              className="flex-1 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 px-3.5 py-2.5 text-xs text-slate-900 dark:text-white placeholder:text-slate-400 focus:border-sky-500 focus:outline-hidden"
            />
            <button
              type="submit"
